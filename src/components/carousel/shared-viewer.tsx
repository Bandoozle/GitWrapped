    ...
    <div className="mx-auto w-full max-w-3xl">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-xl text-muted">{story.description}</p>
        </div>
        <a
          href={story.carousel.githubUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 rounded-md border border-border bg-surface px-4 py-2.5 text-sm transition hover:border-accent"
        >
          GitHub
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        {SLIDE_COMPONENTS.map((slide, i) => (
          <button
            ...
            type="button"
            onClick={() => setIndex(i)}
            className={`rounded-md px-3 py-1.5 text-xs whitespace-nowrap transition ${
              index === i ? "bg-white !text-black" : "bg-surface text-muted"
            }`}
          >
            {i + 1}. {slide.label}
          </button>
      ...
      <div className="mt-5 flex items-center justify-between">
        <button
          ...
          disabled={index === 0}
          className="inline-flex items-center gap-1 text-sm text-muted disabled:opacity-30 hover:text-foreground"
        >
